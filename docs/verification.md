...
| Frontend unit + component | `pnpm test` (vitest) | 465 passed — formatters, store, terminal slice, RTL component tests (cluster switch, table filter/sort, detail tabs, YAML dry run, confirm dialog, terminal open/close, background-cluster isolation), B75 injection contract, B60 saved views, B87 column config + CSV |
